...
HRESULT CD3DFont::InitDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	// Added 
	// If we want the full character set, double the estimated sizes
	if ( m_dwFontFlags & D3DFONT_FULLSET ) 
	{
        m_dwTexWidth  *= 2 ;
		m_dwTexHeight *= 2 ;
	} ; 
	...
	// Added 
	TCHAR MaxChar = ( m_dwFontFlags & D3DFONT_FULLSET ) ? 255 : 127 ; 
	// Changed for full character set support
    for( TCHAR c=32; c < MaxChar ; c++ )
    {
        str[0] = c;
...
HRESULT CD3DFont::GetTextExtent( const TCHAR* strText, SIZE* pSize )
{
    ...
    while( *strText )
    {
		...
		// Changed for full charset support 
		if ( m_dwFontFlags & D3DFONT_FULLSET )
		{
			if( (c-32) < 0 )
				continue;
...
// Batching : to do 
//
HRESULT CD3DFont::BeginText( ) 
{
    return S_OK;
} 

HRESULT CD3DFont::EndText( ) 
{
    return S_OK;
} 
